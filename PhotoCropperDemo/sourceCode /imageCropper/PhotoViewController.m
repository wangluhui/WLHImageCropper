...
//    for (int i=0; i<2; i++) {
    ...
    rotateBtn.frame = CGRectMake(10, 0, 60, CGRectGetHeight(bottomView.frame));
    ...
    [rotateBtn addTarget:self action:@selector(startRotate) forControlEvents:UIControlEventTouchUpInside];
    
}
-(void)startRotate
{
    _rotate += 1/8.f;
    [_imageView setTransform:CGAffineTransformMakeRotation(M_PI*_rotate)];
//    _imageView.frame = CGRectMake(0, 0, 480, 320); //set the frame after the rotate
//    self.contentSize = CGSizeMake(480, 320);
    
   
//    _layer.frame = CGRectMake(SCREEN_WIDTH*0.6+2, _y-_heartFlySize.height*0.2-5,  _heartFlySize.width*0.2,  _heartFlySize.height*0.2);
